...
{
    $.ajax({
        url: "/tms/vtusettlement/transactiondetails/" + id,
        async: true,
        dataType: 'json',
        success: function (json) {
            if(json !== null && typeof json !== 'undefined')
            {
                ...
                use += "PROCESSOR VAT: <span style=\"float:right;\">NGN " + records[0].varetzn + "</span></br>";
                use += "USER CREDENTIAL: <span style=\"float:right;\">" + records[0].destination + "</span></br></br></br>";
                use += records[0].responsetxn + "</br></br></br>";
    ...
    swal({
        ...
        if (result.value) {
            ...
            }else
            {
                ...
                $.ajax({
                    type: "POST",
                    url : "/tms/vtusettlement/getbydaterange",
                    data : fd,
                    processData: false,
                    contentType: false,
                    ...
                    success : function(json) {
                        ...
                        if(records !== null && typeof records !== 'undefined')
                        {
                            ...
                            for(var i = 0; i < records.length; i++)
                            {
                                ...
                                arr = [];
                                if(records[i].transtype !== "VTU" && records[i].transtype !== "BILLSPAYMENT")
                                    continue;
...
function proceedPlease() 
{
	...
	$.ajax({
        url: "/tms/vtusettlement/getAllToday/" + uLastId,
        async: true,
        dataType: 'json',
        success: function (json) {
            if(json !== null && typeof json !== 'undefined')
...
function getAllAgency()
{
    $.ajax({
        type: "GET",
        url : "/tms/vtusettlement/getallagency",
        processData: false,
        contentType: false,
...
function getAllTerminals()
{
    $.ajax({
        type: "GET",
        url : "/tms/vtusettlement/getalltids",
        processData: false,
        contentType: false,
