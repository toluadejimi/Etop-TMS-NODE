...
function gotoEdit(id)
{
    ...
    $("#friendlyname").val(record[id].friendlyname);
    $("#ssl").val(record[id].ssl.toString()).change();
    $("#mestype").val(record[id].mestype.toString()).change();
...
$("#demo-form2" ).submit(function( event ) {
    ...
    fd.append('friendlyname', $("#friendlyname").val());
    fd.append('ssl', $("#ssl").val());
    fd.append('mestype', $("#mestype").val());
...
function gotoDelete(id)
{
    swal({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: "DELETE",
                url : "/tms/settings/deletehost/" + record[id].id,
                data : fd,
                processData: false,
                contentType: false,
