                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        if(dif >= 1)
                        {
                            ...
                            logger.info("Spitting out all cards to: " + req.clientIp + ". Time: " + new Date().toLocaleString());
                            return res.status(200).render("transaction/ejournal", {details: JSON.stringify(response), role: role, usertype, usertype});
                        }
                    }
                }
            });
...
{
    console.log("Inside get all card transactions");
    ...
    try
    {
        ...
        pool.query(qry, [token, username], (err, result) => { 
            ...
            else
            {
                ...
                {
                    const txn = "SELECT * FROM ejournal WHERE current_date_uzoezi BETWEEN $1 AND $2 order by id desc";
                    pool.query(txn, [startdate, enddate], (err,  comms) => {    
                        ...
                        else
                        {
                            ...
                            logger.info("xxxLength of data: " + comms.rows.length);
