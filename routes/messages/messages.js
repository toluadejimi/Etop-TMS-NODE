            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        return res.status(200).render("messages/messages", {details: JSON.stringify(response), role: role, usertype, usertype});
                    }
                }
            });
        }catch(e)
    ...
    try
    {
        ...
        pool.query(qry, [token, username], (err, result) => { 
            ...
            else
            {
                ...
                {
                    const txn = "SELECT * FROM messages order by id desc";
                    ...
                    }else
                    {
                        ...
                        pool.query(qry2, [id], (err, resul) => {
                            if (err) 
                            {
                                ...
                                res.status(500).send({"status": 500, "message": "Cannot Delete Messages Details. Retry Later or Contact Admin."});
                            }else
                            {
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    }else
                    {
                        ...
                        {
                            var qry2 =
                                "UPDATE messages SET message = $1, walletids = $2 WHERE id = $3";
                            pool.query(qry2, [req.body.message, req.body.walletids, req.body.id], (err, resul) => {
                                if (err) 
                                {
                                    console.log(err)
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Update Messages Details. Retry Later or Contact Admin."});
                                }else
                                {
                                    return res.status(200).send({"status": 200, "message": "Update Successful."});
                                }
                        ...
                        {
                            var qry2 = "INSERT INTO messages " + 
                                "(message, walletids, priority) " + 
                                "VALUES ($1, $2, $3)";
                            pool.query(qry2, [req.body.message, req.body.walletids, "HIGH"], (err, resul) => {
                                if (err) 
                                {
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
