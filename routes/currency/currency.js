            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        return res.status(200).render("currency/currency", {details: JSON.stringify(response), role: role, usertype, usertype});
                    }
                }
            });
        }catch(e)
        ...
        pool.query(qry, [req.cookies.token_tcm, req.cookies.username], (err, result) => {
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    {
                        var qry2 = "DELETE FROM currency where id = $1";
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    }else
                    {
                        ...
                        {
                            var qry2 =
                                "UPDATE currency SET name = $1, abbreviation = $2, code = $3, minorunit = $4, remarks = $5," + 
                                "datemodified = $6, namemodified = $7 WHERE id = $8";
                            pool.query(qry2, [req.body.name, req.body.abbreviation, req.body.code,
                                req.body.minorunit, req.body.remarks,
                                datetime(), result.rows[0].fullname, req.body.id], (err, resul) => {
                                if (err) 
                                {
                                    console.log(err)
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Update Currency Details. Retry Later or Contact Admin."});
                                }else
                                {
                        ...
                        {
                            var qry2 = "INSERT INTO currency " + 
                                "(name, abbreviation, code, minorunit, remarks," +
                            ...
                            pool.query(qry2, [req.body.name, req.body.abbreviation, req.body.code,
                                ...
                                {
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Insert Currency Details. Retry Later or Contact Admin."});
                                }else
                                {
