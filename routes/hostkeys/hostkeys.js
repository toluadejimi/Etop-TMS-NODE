            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        return res.status(200).render("hostkeys/hostkeys", {details: JSON.stringify(response), role: role, usertype, usertype});
                    }
                }
            });
        }catch(e)
...
router.get("/getallhostkeys", function(req, res)
{
    try
    {
        var str = new Date().toLocaleString();
            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        var qry2 = "SELECT * FROM keys where ifavailable = $1";
...
router.delete("/deletehostkey/:id", function(req, res)
{
    try
    {
        ...
        pool.query(qry, [req.cookies.token_tcm, req.cookies.username], (err, result) => {
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    {
                        var qry2 = "DELETE FROM keys where id = $1";
...
router.post("/hostkey", function(req, res)
{
    try
    {
        var qry = "SELECT * FROM tokens WHERE token = $1 AND username = $2";
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    }else
                    {
                        ...
                        {
                            var qry2 =
                                "UPDATE keys SET name = $1, component1 = $2, component2 = $3, maker = $4, ifavailable = $5, datemodified = $6, namemodified = $7"
                                + " WHERE id = $8";
                            pool.query(qry2, [req.body.name, req.body.component1, req.body.component2,
                                req.cookies.username, 
                                "false", datetime(), result.rows[0].fullname, req.body.id], (err, resul) => {
                                if (err) 
                                {
                                    console.log(err)
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Update Keys Details. Retry Later or Contact Admin."});
                                }else
                                {
                        ...
                        {
                            var qry2 = "INSERT INTO keys " + 
                                "(name, component1, component2, maker, addedby, ifavailable, datecreated, namecreated) " + 
                                "VALUES ($1, $2, $3, $4, $5, $6, $7, $8)";
                            pool.query(qry2, [req.body.name, req.body.component1, req.body.component2, req.cookies.username,
                                req.cookies.username,
                            "false", datetime(), result.rows[0].fullname], (err, resul) => {
                                if (err) 
                                {
                                    console.log(err);
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Insert Keys Details. Retry Later or Contact Admin."});
                                }else
                                {
