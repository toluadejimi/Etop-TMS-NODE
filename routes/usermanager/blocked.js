            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        return res.status(200).render("user/blocked", {details: JSON.stringify(response), role: role, usertype, usertype});
                    }
                }
            });
        }catch(e)
...
router.get("/getallblocked", function(req, res)
{
    try
    {
        var str = new Date().toLocaleString();
            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        var qry2 = "SELECT * FROM etop_users WHERE status = $1";
                        pool.query(qry2, ["blocked"], (err, resul) => {
                            if (err) 
                            {
...
router.put("/removeblock", function(req, res)
{
    try
    {
        var qry = "SELECT * FROM tokens WHERE token = $1 AND username = $2";
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    }else
                    {
                        ...
                        + " WHERE username = $3";
                        pool.query(qry2, ["active", "", req.body.username], (err, resul) => {
                            if (err) 
                            {
                                logger.info("Database Issue. User: " + username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                res.status(500).send({"status": 500, "message": "Cannot Signup. Retry Later."});
