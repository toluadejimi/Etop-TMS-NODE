            ...
            pool.query(qry, [token, username], (err, result) => { 
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        return res.status(200).render("banks/banks", {details: JSON.stringify(response), role: role, usertype, usertype});
                    }
                }
            });
        }catch(e)
        ...
        pool.query(qry, [req.cookies.token_tcm, req.cookies.username], (err, result) => {
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    {
                        var qry2 = "DELETE FROM banks where id = $1";
            ...
            }else
            {
                ...
                }else
                {
                    ...
                    }else
                    {
                        ...
                        {
                            var qry2 =
                                "UPDATE banks SET name = $1, code = $2, remarks = $3," + 
                                "datemodified = $4, namemodified = $5 WHERE id = $6";
                            pool.query(qry2, [req.body.name, req.body.code, req.body.remarks,
                                datetime(), result.rows[0].fullname, req.body.id], (err, resul) => {
                                if (err) 
                                {
                                    console.log(err)
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Update Bank Details. Retry Later or Contact Admin."});
                                }else
                                {
                        ...
                        }else
                        {
                            ...
                            pool.query(qry2, [req.body.name, req.body.code, req.body.remarks,
                                ...
                                {
                                    logger.info("Database Issue. User: " + req.cookies.username + ". Ip: " + req.clientIp + ". Time: " +  new Date().toLocaleString());
                                    res.status(500).send({"status": 500, "message": "Cannot Insert Bank Details. Retry Later or Contact Admin."});
                                }else
                                {
